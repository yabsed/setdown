...
type Options = {
  desktop: DesktopPort; tabs: WorkspaceTab[]; active: () => WorkspaceTab | null; activeId: () => string | null;
  text: () => string | null; lineCount: () => number; reader: ReaderController;
};
export class PreviewSession {
  ...
  private readonly listeners = new Set<(payload: { tabId: string; message: Record<string, unknown> }) => void>();
  ...
  private createCoordinator() { return new PreviewRenderCoordinator((revision) => this.render(revision)); }
  cancelSchedule() {
  ...
  newSession() { this.reset(); this.coordinator = this.createCoordinator(); }
  schedule(targetRevision: number) {
    if (!hasMarkdownPreview(this.options.active()?.document)) return;
    const checkpoint = () => {
      const tab = this.options.active();
      if (tab?.surface === 'editor' && hasMarkdownPreview(tab.document)
        && tab.revision >= targetRevision) void this.ensure(tab.revision);
    };
    this.timer ??= window.setTimeout(() => { this.timer = null; checkpoint(); }, 500);
    if (this.idleTimer !== null) window.clearTimeout(this.idleTimer);
    this.idleTimer = window.setTimeout(() => { this.idleTimer = null; checkpoint(); }, 150);
  }
  updateUi() {
    const tab = this.options.active();
    const refreshing = this.options.reader.awaiting || (tab?.surface === 'viewer'
      && this.coordinator.isRendering && this.readyRevision !== tab.revision);
    view.rendering = !!refreshing;
    view.renderVariant = this.options.reader.awaiting || this.readyRevision === null ? 'blocking' : 'refresh';
  ...
  private async render(targetRevision: number) {
    ...
    if (!tab || !tabId || !hasMarkdownPreview(tab.document) || targetRevision !== tab.revision) return false;
    ...
    try {
      const result = await this.options.desktop.preparePreview(tabId, text, targetRevision, documentPath, theme);
      ...
      if (rendered && this.options.reader.themeId !== theme) {
        ...
        if (this.options.reader.themeId === assets.themeId) this.options.reader.applyAssets(rendered, assets);
      }
      ...
      this.error = { revision: targetRevision, message: error instanceof Error ? error.message : String(error) };
      return false;
    }
  }
  private isCurrent(epoch: number, tabId: string, documentPath: string) {
    const active = this.options.active();
    return epoch === this.epoch && this.options.activeId() === tabId && active?.document.path === documentPath;
  }
  async ensure(revision: number) {
    if (!hasMarkdownPreview(this.options.active()?.document)) return false;
  ...
  position(target: ViewportAnchor, revision: number, tabId = this.options.activeId() ?? '',
    lineCount = this.options.lineCount(), settle = true, band: BandLine[] = []): Promise<boolean> {
    const tab = this.options.tabs.find((candidate) => candidate.id === tabId);
    if (tab && !hasMarkdownPreview(tab.document)) return Promise.resolve(false);
    const revealed = clampAnchor(target, lineCount);
    const requestId = ++this.positionRequest;
    return new Promise((resolve) => {
      const cleanup = () => { this.listeners.delete(handle); window.clearTimeout(timeout); };
      const handle = (payload: { tabId: string; message: Record<string, unknown> }) => {
        const message = payload.message;
        if (payload.tabId !== tabId || message.source !== 'crossnote' || message.type !== 'marktex:preview-positioned'
          || message.revision !== revision || message.requestId !== requestId) return;
        cleanup(); resolve(true);
      };
      const timeout = window.setTimeout(() => { cleanup(); resolve(false); }, 1000);
      this.listeners.add(handle);
      this.options.reader.send(tabId, { command: 'marktex:position-preview', sourceLine: revealed.sourceLine,
        topRatio: revealed.yRatio, band, requestId, settle });
    });
  }
