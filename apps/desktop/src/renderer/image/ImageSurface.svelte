<script lang="ts">
  ...
  let zoom = $state<number | 'fit'>(initial?.zoom ?? 'fit');
  ...
  const scale = $derived(zoom === 'fit' ? Math.min(1, Math.max(1, viewportWidth - 32) / (rotatedWidth || 1),
  ...
  function changeZoom(value: string) {
    remember(); zoom = value === 'fit' ? 'fit' : Number(value); void layout();
  }
  ...
  onMount(() => {
    ...
    return () => { dead = true; observer.disconnect(); if (url) URL.revokeObjectURL(url); };
  });
</script>
...
<style>
  ...
  .image-toolbar { display: flex; align-items: center; gap: 10px; padding: 6px 10px; border-bottom: 1px solid var(--app-border); font-size: 12px; background: var(--app-chrome); }
