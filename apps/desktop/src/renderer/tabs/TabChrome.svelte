...
<nav class="tab-strip" aria-label="Open documents">
  <div class="tab-list" role="tablist">
    {#each view.tabs as tab (tab.id)}
      <button
        ...
        type="button"
        role="tab"
        aria-selected={tab.active && !diffOpen}
        title={tab.path}
        draggable="true"
        data-tab-id={tab.id}
    ...
    {/each}
    {#if diffOpen}
      <button class="document-tab git-diff-tab" type="button" role="tab" aria-selected="true"
        title={diffPath}>
  ...
  <div class="tab-actions">
    ...
    <button class="editor-action insert-table-button" type="button" hidden={diffOpen}
    ...
    <button class="editor-action insert-link-button" type="button" hidden={diffOpen}
    ...
    <button class="viewer-action toc-toggle" class:is-active={view.tocOpen} type="button" hidden={diffOpen}
    ...
    <span class="tab-action-divider" aria-hidden="true" hidden={diffOpen}></span>
    <button class="mode-toggle" type="button"
      hidden={(!project.gitDiff && view.surface === 'empty') || project.gitDiffLoading}
      title={`${project.gitDiff
      ...
      aria-label={project.gitDiff
        ? project.gitDiffMode === 'rendered' ? 'View Source Diff' : 'View Rendered Diff'
        : view.surface === 'viewer' ? 'Switch to Editor' : 'Switch to Viewer'}
      onclick={actions.toggleSurface}>
      {#if project.gitDiff ? project.gitDiffMode === 'rendered' : view.surface === 'viewer'}
