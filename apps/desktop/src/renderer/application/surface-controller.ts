...
export class SurfaceController {
  ...
  enterViewer = async (): Promise<void> => {
    const tab = this.options.workspace.active;
    if (!this.options.editor.loaded || !tab) return;
    ...
    if (readyRevision !== null) {
      void this.options.preview.position(
        anchor,
        readyRevision,
        tab.id,
        this.options.editor.lineCount(tab),
        false,
        viewport.band,
      );
    }
  ...
  requestViewerAnchor(): void {
    ...
    this.options.reader.send(pendingTabId, {
  ...
  editorScrolled = (): void => {
    ...
    if (session.surface !== 'editor' || !workspace.activeId) return;
    ...
    this.editorPositionFrame = window.requestAnimationFrame(() => {
      ...
      if (session.surface !== 'editor' || !tab || !workspace.activeId) return;
      ...
      reader.send(workspace.activeId, {
