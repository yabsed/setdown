...
const content = new ContentController({
  sourceAtlas,
  applyDisclosures: reader.applyDisclosures,
  scheduleHeadings: reader.scheduleHeadings,
  viewportChanged: viewport.schedule,
});
...
installCommandRouter({
  ...
  viewport,
  send,
  clearSearch: reader.clearSearch,
  publishHeadings: reader.publishHeadings,
  search: reader.search,
  scrollToHeading: reader.scrollToHeading,
});

// Full Crossnote runtime이 기대하는 host API. Lean runtime은 아래에서 직접 설치한다.
