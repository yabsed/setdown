...
export class ExplorerService {
  ...
  async readDirectory(state: WindowState, candidate: string): Promise<ProjectEntry[]> {
    ...
    return entries
      ...
      .map((entry): ProjectEntry => this.entry(
  ...
  async move(
    ...
    if (directory === from || directory.startsWith(`${from}${path.sep}`)) {
  ...
  async trashEntry(state: WindowState, entryPath: string): Promise<void> {
    await this.trash(this.mutableEntry(state, entryPath));
  }
  ...
  private entry(entryPath: string, directory: boolean): ProjectEntry {
    return {
      ...
      kind: directory ? 'directory' : isMarkdownDocument(entryPath) ? 'document' : 'file',
    };
  }

  private async ensureMissing(candidate: string): Promise<void> {
    if (await fs.stat(candidate).catch(() => null)) {
      throw new Error(`“${path.basename(candidate)}” already exists.`);
    }
  }
}
