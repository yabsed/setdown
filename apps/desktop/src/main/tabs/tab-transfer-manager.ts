...
export class TabTransferManager {
  ...
  private register(sourceId: number, transferId: unknown, candidate: unknown) {
    const tab = candidate as TransferableTab | null;
    if (typeof transferId !== 'string' || !tab || typeof tab.id !== 'string') return;
    const preview = this.options.previews.views.get(tab.id)?.view.webContents;
  ...
  private async adopt(destinationId: number, transferId: string) {
    ...
    preview.view.setBackgroundColor(previewThemeBackground(
      transfer.tab.previewTheme ?? this.options.theme(),
    ));
    ...
    preview.pendingScrollRatio = keepsGeometry || band.length
  ...
  private async claim(destinationId: number, transferId: string) {
    ...
    if (transfer.sourceWebContentsId === destinationId || transfer.claimedByWebContentsId !== null) {
      return null;
    }
  ...
  private release(sourceId: number, transferId: string) {
    ...
    const destination = transfer.claimedByWebContentsId === null
      ? null : this.options.stateFor(transfer.claimedByWebContentsId)?.window;
    this.transfers.delete(transferId);
      ...
      destination.once('show', () => setImmediate(() => {
        for (const preview of this.options.previews.views.values()) {
          if (preview.ownerWebContentsId === destination.webContents.id) {
            this.options.previews.restoreScroll(preview);
          }
        }
  ...
  private detach(sourceId: number, transferId: string, x: unknown, y: unknown) {
    const transfer = this.transfers.get(transferId);
    if (!transfer || transfer.sourceWebContentsId !== sourceId
      || transfer.claimedByWebContentsId !== null) return;
    ...
    const destination = transfer.preparedWindow && !transfer.preparedWindow.isDestroyed()
      ...
      : this.options.createWindow(transfer.detachPosition, null, false, this.contentSize(sourceId));
    ...
    const sendIncoming = async () => {
      ...
      const band = await transfer.previewBand.catch(() => [] as BandLine[]);
      if (!this.transfers.has(transferId) || destination.isDestroyed()) return;
      destination.webContents.send('tabs:transfer-incoming', {
        transferId,
        tab: {
          ...transfer.tab,
          viewerBand: band,
          previewGeometryUnchanged: this.keepsGeometry(transfer, destination.webContents.id),
        },
      });
    };
    if (destination.webContents.isLoadingMainFrame()) {
      destination.webContents.once('did-finish-load', () => void sendIncoming());
    } else void sendIncoming();
  }

  registerIpc() {
    ipcMain.on('tabs:register-transfer', (event, { transferId, tab }) =>
      this.register(event.sender.id, transferId, tab));
    ...
    ipcMain.on('tabs:complete-transfer', (event, id) => {
      ...
      if (!transfer || transfer.claimedByWebContentsId !== event.sender.id
        || !transfer.previewAdopted) return;
    ...
    ipcMain.on('tabs:detach-to-window', (event, { transferId, x, y }) =>
      this.detach(event.sender.id, transferId, x, y));
  }
}
