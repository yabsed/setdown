...
test('warm Esc presents actual content while the next installation is held', async () => {
  ...
  try {
    ...
    await expect.poll(async () => (await reviews(app!)).length, { timeout: 30_000 }).toBe(1);
    ...
    await app.evaluate(({ webContents }, ids) => {
      ...
      for (const id of ids) {
        ...
        contents.send = (channel: string, ...args: unknown[]) => {
          ...
          if (channel === 'preview:command' && message?.command === 'marktex:update-html'
            && message.html?.includes('held-installation')) { state.held.push(() => send(channel, ...args)); return; }
          send(channel, ...args);
        };
