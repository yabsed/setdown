<script lang="ts">
  import { emitUiCommand } from '../ui-command';
...
<nav class="tab-strip" aria-label="열린 문서" hidden={view.tabs.length === 0}>
  <div class="tab-list" role="tablist">
    {#each view.tabs as tab (tab.id)}
      <button
        ...
        draggable="true"
        data-tab-id={tab.id}
        onclick={() => emitUiCommand({ type: 'activate-tab', tabId: tab.id })}
        ondragstart={(event) => emitUiCommand({ type: 'tab-drag-start', tabId: tab.id, event })}
        ondragend={(event) => emitUiCommand({ type: 'tab-drag-end', tabId: tab.id, event })}
      >
        ...
        <span
          ...
          onclick={(event) => {
            ...
            emitUiCommand({ type: 'close-tab', tabId: tab.id });
          }}
          onkeydown={(event) => {
  ...
  <div class="tab-actions">
    ...
    <button class="mode-toggle" type="button" hidden aria-label="편집기로 전환">
